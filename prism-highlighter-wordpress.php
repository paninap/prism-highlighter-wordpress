...
/**
  ...
  * @author             techmagus
  * @license            GPL-3.0
  * @copyright          Copyright (c) 2016 JC John Sese Cuneta
  *
  * @wordpress-plugin
  * Plugin Name:        Prism Highlighter for WordPress
  * Plugin URI:         https://techmagus.ninja/prism-highlighter-wordpress/
  * Description:        A minimal and modular Prism WordPress plugin with a "detached" option.
  * Version:            1.0.0
  * Author:             techmagus
  * Author URI:         https://techmagus.ninja
  * License:            GPLv3
  * License URI:        https://www.gnu.org/licenses/gpl-3.0.html
  * GitHub Plugin URI:  https://github.com/Paninap/prism-highlighter-wordpress
  *
  * Based on: https://github.com/nextgenthemes/prism
  ...
  public function shortcode( $atts, $content = null ) {
    ...
    if ( $url ) {
      if ( false === filter_var( $url, FILTER_VALIDATE_URL ) ) {
        return sprintf( '<p><b>Prism Shortcode Error:</b> URL <code>%s</code> is invalid </p>', esc_html( $url ) );
      }

      $response = wp_remote_get( esc_url( $url ) );

      if ( is_wp_error( $response ) ) {
        return sprintf( '<p><b>Prism Shortcode Error:</b> could not get remote content. WP_Error message:<br>%s</p>', esc_html( $response->get_error_message() ) );
      }
      elseif( 200 != $response['response']['code'] ) {
        return sprintf( '<p><b>Prism Shortcode Error:</b> could not get remote content. HTTP response code %s</p>', esc_html( $response['response']['code'] ) );
      }
    ...
    if ( ! $field ) {
      return '<p><b>Prism Shortcode Error:</b> field, url, data_src is missing</p>';
    }
    ...
    if ( empty( $field_content ) ) {
      return '<p><b>Prism Shortcode Error:</b> Custom field not set or empty</p>';
    }
